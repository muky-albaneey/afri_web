  ...
  return (
    <div className="w-full flex justify-center">
      <div className="relative w-full max-w-4xl">
        <div className="hidden w-full bg-[#E5D2C1] rounded-xl flex flex-col md:flex-row items-center md:items-start px-6 py-8 md:p-10 gap-6">
          {/* Left Image */}
          ...
          <div className="flex-1 text-center md:text-left">
            ...
            <button className="mt-4 bg-[#8C5A2B] text-white px-5 py-2 rounded-md">Shop Now</button>
          </div>
        ...
        <div className="md:hidden flex flex-col items-center mt-6 gap-4">
          ...
          <Image src={slides[current].image1} width={150} height={150} alt="Sale Gift" />
            ...
            <button className="mt-6 bg-[#8C5A2B] text-white px-5 py-3 rounded-md w-full font-semibold">
                Button
            </button>
        </div>
  ...
  <IoIosArrowBack />
        ...
        <div className="flex justify-center gap-2 mt-4">
          {slides.map((_, index) => (
            <div
              key={index}
              className={`w-3 h-3 rounded-full ${index === current ? "bg-[#8C5A2B]" : "bg-gray-300"}`}
            />
          ))}
        </div>
        <button
